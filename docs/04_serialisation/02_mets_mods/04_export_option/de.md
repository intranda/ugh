---
title: Erweiterte Möglichkeiten des METS-Exports
published: true
keywords:
    - UGH
    - METS
...
* Das Attribut `contentIds` hingegen dient der Referenzierung der einzelnen Seiten der physischen Struktur. Momentan wird es als Pfad zur einzelnen Datei genutzt, an den noch der Dateiname angehängt wird (Dieses Feature ist noch experimentell).
* Wird am Ende eines solchen Metadatums ein regulärer Ausdruck (in Perl5-Syntax) übergeben – in der Form `$REGEXP(s///)` – dann wird dieser reguläre Ausdruck auf den gesamten Wert angewendet, bevor er in das METS übernommen wird.
