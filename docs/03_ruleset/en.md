...
The rule set itself is implemented as an XML file. The root node in each ruleset is the `<Preferences>` element. Within this `<Preferences>` element are all definitions of the metadata and structure types. It is important to note that metadata types must be defined before groupings and structure types.
